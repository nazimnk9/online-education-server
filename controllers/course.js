...
export const uploadImage = async (req, res) => {
  ...
  try {
    ...
    //prepare the image
    const base64Data = new Uint8Array(
    ...
    const snapshot = await uploadBytes(imageRef, base64Data, metadata);
    const location = await getDownloadURL(imageRef);
...
export const removeImage = async (req, res) => {
  try {
    ...
    // delete the image
    const storageRef = ref(storage, `${image.metadata.name}`);
...
export const uploadVideo = async (req, res) => {
  try {
    ...
    // video params
    const fileName = `${nanoid()}.${video.type.split("/")[1]}`;
    ...
    // upload to Firebase Storage
    const snapshot = await uploadBytes(videoRef, readFileSync(video.path), metadata);
    const location = await getDownloadURL(videoRef);
    snapshot.location = location;
    console.log(snapshot);
    res.send(snapshot);
...
export const removeVideo = async (req, res) => {
  try {
    ...
    //console.log("remove video ====>",metadata);
    //return;
...
export const checkEnrollment = async (req, res) => {
  const { courseId } = req.params;
  //find course currently logged in user
  const user = await User.findById(req.user._id).exec();
  // if course id is found user courses array
  let ids = [];
